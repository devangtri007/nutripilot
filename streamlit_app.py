...
def _resolve_recipe_id(meal, recipes):
    """Resolve a persisted recipe_id or legacy/display recipe name safely."""
    lookup = recipes.set_index("recipe_id")
    raw_id = meal.get("recipe_id")

    # IDs may arrive from older session state as non-string scalar values.
    if raw_id is not None:
        candidate_id = str(raw_id).strip()
        if candidate_id in lookup.index:
            return candidate_id

    # Support both legacy `name` and any intermediate `recipe_name` field.
    names = [
        meal.get("name"),
        meal.get("recipe_name"),
        meal.get("recipe", {}).get("name") if isinstance(meal.get("recipe"), dict) else None,
    ]
    name_to_ids = {}
    ...
    # Last-resort safe match: exact normalized token containment, only when
    # exactly one catalogue recipe matches. This handles harmless punctuation
    # or suffix differences without guessing between similar recipes.
    for name in names:
        key = _normalize_recipe_text(name)
        if not key:
            continue
...
def normalize_plan_recipe_ids(plan, recipes, foods):
    """Make plans from older session-state versions compatible with refinement.

    Refinement must operate on stable catalogue IDs. Older plans can contain
    only a displayed recipe name, and names can differ slightly because of
    punctuation/formatting. Resolve those names deterministically, then
    recalculate nutrition from the current catalogue rather than trusting
    stale session-state nutrition values.
    """
    lookup = recipes.set_index("recipe_id")
    normalized_days = []

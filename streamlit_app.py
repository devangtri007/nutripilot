...
def _normalize_recipe_text(value):
    """Normalize recipe names so old/session-state display variants still resolve."""
...
def _resolve_recipe_id(meal, recipes):
    """Resolve a stable catalogue recipe_id from ID or legacy/display fields.

    Refinement must never depend on the exact formatting of a displayed recipe
    name. Older plans can contain only `name`/`recipe_name`, while newer plans
    persist `recipe_id`. Resolution is deterministic and refuses ambiguous
    matches rather than guessing.
    """
    lookup = recipes.set_index("recipe_id")

    # 1) Stable ID wins whenever it exists in the current catalogue.
    ...
    # 2) Collect every known legacy/display name field.
    names = [
        meal.get("name"),
        meal.get("recipe_name"),
        meal.get("recipe", {}).get("name")
        if isinstance(meal.get("recipe"), dict)
        else None,
    ]
    ...
    for recipe_id, row in lookup.iterrows():
        key = _normalize_recipe_text(row.get("name"))
        if key:
            name_to_ids.setdefault(key, []).append(str(recipe_id))

    for name in names:
        key = _normalize_recipe_text(name)
        if key and len(name_to_ids.get(key, [])) == 1:
            return name_to_ids[key][0]

    # 4) Conservative containment fallback for harmless suffix differences.
    for name in names:
        key = _normalize_recipe_text(name)
        if not key:
            continue
        ...
        for recipe_id, row in lookup.iterrows():
            ...
            if key == recipe_key or key in recipe_key or recipe_key in key:
                matches.append(str(recipe_id))
        if len(matches) == 1:
            return matches[0]
...
def normalize_plan_recipe_ids(plan, recipes, foods):
    """Normalize current/legacy plans into the current recipe schema.

    This is intentionally called immediately before refinement. It repairs
    session-state plans created by older NutriPilot versions and re-verifies
    nutrition from the currently deployed food catalogue.
    """
    lookup = recipes.set_index("recipe_id")
    normalized_days = []

...
def _deterministic_refinement_changes(user_request, profile, recipes, foods, recipe_candidates, current_plan):
    """Create safe changes for simple, testable constraints without relying on LLM behavior."""
    intent = _refinement_intent(user_request)
    changes = []
    lookup = recipes.set_index("recipe_id")
    forbidden = refinement_forbidden_ingredients(user_request)

    # Higher-protein: explicitly choose a catalogue recipe with the highest
    # verified protein for the requested meal/day, but only if it improves the
    # current meal. This fixes the case where the model returns an empty change
    # list because protein values were not directly exposed in its prompt.
    if intent["higher_protein"]:
        target_meals = []
        day_match = re.search(r"\bday\s*(\d+)\b", str(user_request), re.I)
        meal_match = re.search(r"\b(breakfast|lunch|dinner|snack(?:s)?)\b", str(user_request), re.I)
        target_day = int(day_match.group(1)) if day_match else None
        target_meal = meal_match.group(1).title() if meal_match else None
        if target_meal == "Snacks":
            target_meal = "Snacks"
        for day in plan_days(current_plan):
            ...
            for meal in day.get("meals", []):
                ...
                if target_meal and mt.casefold() != target_meal.casefold():
                    continue
                target_meals.append((day, meal))

        for day, meal in target_meals:
            mt = meal.get("meal_type") or meal.get("meal")
            current_id = meal.get("recipe_id")
            if current_id in lookup.index:
                current_nutrition = calculate_recipe_nutrition(lookup.loc[current_id].to_dict(), foods)["nutrition"]
                current_protein = float(current_nutrition.get("protein_g", 0))
            else:
            ...
            for _, row in pool.iterrows():
                ...
                scored.append((float(verified["nutrition"].get("protein_g", 0)), str(row["recipe_id"])))
            scored.sort(reverse=True)
            if scored and scored[0][0] > current_protein + 0.1:
                changes.append({"day": int(day["day"]), "meal": mt, "recipe_id": scored[0][1],
                                "reason": f"Higher verified protein: {scored[0][0]:.1f} g vs {current_protein:.1f} g."})

    # Time constraint: replace every meal that exceeds the requested limit.
    if intent["time_limit"]:
        ...
        for day in plan_days(current_plan):
            for meal in day.get("meals", []):
                ...
                if forbidden:
                    pool = pool.loc[~pool.apply(lambda row: recipe_contains_any(row, forbidden), axis=1)]
                pool = pool[~pool["recipe_id"].isin([c["recipe_id"] for c in changes])]
                if not pool.empty:
                    pool = pool.copy()
                    pool["_time"] = pd.to_numeric(pool["total_time_min"], errors="coerce")
                    rid = pool.sort_values("_time").iloc[0]["recipe_id"]
                    if not any(c["day"] == int(day["day"]) and c["meal"] == mt for c in changes):
                        changes.append({"day": int(day["day"]), "meal": mt, "recipe_id": rid,
...
st.caption("Build: Refinement V4")

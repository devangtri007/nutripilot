...
FOOD_ALIAS_PATH = "data/food_aliases.csv"
...
@st.cache_data
def load_food_aliases(file_version=None):
    if not os.path.exists(FOOD_ALIAS_PATH):
        return pd.DataFrame(columns=["alias", "canonical_food", "source", "confidence", "notes"])
    return pd.read_csv(FOOD_ALIAS_PATH)


@st.cache_data
def load_product_data(file_version=None):
    if not os.path.exists(PRODUCT_DATA_PATH):
        return pd.DataFrame()
    return pd.read_csv(PRODUCT_DATA_PATH, low_memory=False)


def resolve_food_name(food_name, foods=None):
    """Resolve an ingredient through exact match, then the governed alias table."""
    candidate = str(food_name).strip()
    if foods is None:
        foods = globals().get("foods")
    if foods is not None and candidate in set(foods["food"].astype(str)):
        return candidate

    aliases = globals().get("food_aliases")
    if aliases is not None and not aliases.empty:
        normalized = candidate.casefold().strip()
        matches = aliases[aliases["alias"].astype(str).str.casefold().str.strip().eq(normalized)]
        if len(matches) == 1:
            canonical = str(matches.iloc[0]["canonical_food"]).strip()
            if foods is None or canonical in set(foods["food"].astype(str)):
                return canonical
    return None
...
def build_recipe_candidates(recipes, foods, profile, context):
    ...
    for _, recipe in recipes.iterrows():
        ...
        missing = [x["food"] for x in ingredients if resolve_food_name(x["food"], foods) is None]
        if missing:
            continue
...
def validate_catalogue_integrity(foods, recipes):
    ...
    for _, recipe in recipes.iterrows():
        for item in parse_ingredients(recipe["ingredients"]):
            food = item["food"]
            if resolve_food_name(food, foods) is None:
...
def calculate_recipe_nutrition(recipe, foods):
    ...
    for item in ingredients:
        requested_food = item["food"]
        food = resolve_food_name(requested_food, foods)
        grams = float(item["grams"])

        if food is None or food not in lookup.index:
            raise ValueError(
                f"Recipe '{recipe.get('name', recipe.get('recipe_id', 'unknown'))}' "
                f"contains '{requested_food}', but that ingredient is missing from "
...
def calculate_scaled_recipe_nutrition(recipe, foods, serving_multiplier=1.0):
    ...
    for item in parse_ingredients(recipe["ingredients"]):
        ...
        if food is None or food not in lookup.index:
            raise ValueError(
                f"Recipe '{recipe.get('name', recipe.get('recipe_id', 'unknown'))}' "
                f"contains '{requested_food}', but that ingredient is missing from data/foods.csv."
            )
...
food_aliases = load_food_aliases(_data_file_version(FOOD_ALIAS_PATH))
products = load_product_data(_data_file_version(PRODUCT_DATA_PATH))
...
if profile and context:
    ...
    with st.expander("Food data provenance"):
        ...
        c1.metric("Ingredients", f"{len(foods):,}")
        c2.metric("Aliases", f"{len(food_aliases):,}")
        c3.metric("Products", f"{len(products):,}" if not products.empty else "0")
        st.caption(
        ...
        st.caption(
            "USDA FoodData Central is the scalable public-domain/CC0 source. "
            "ICMR-NIN IFCT 2017 is the Indian reference; use an approved structured "
            "IFCT export rather than scraping or redistributing the PDF."
        )
